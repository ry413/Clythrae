...
function buf2string(buffer: any) {
  var arr: any = Array.prototype.map.call(new Uint8Array(buffer), x => x);
  var str = '';
...
function getCharCodeat(str: string) {
  var list = [];
...
//判断返回的数据是否加密
function isEncrypt(fragNum: any, list: any, md5Key: any) {
...
function getSecret(deviceId: any, serviceId: any, characteristicId: any, client: any, kBytes: any, pBytes: any, gBytes: any, data: any) {

  var obj: any = [], frameControl = 0;
  sequenceControl = parseInt(sequenceControl as any) + 1;
...
function writeDeviceRouterInfoStart(deviceId: any, serviceId: any, characteristicId: any, data: any) {
  var obj: any = {},
    frameControl = 0;
  sequenceControl = parseInt(sequenceControl as any) + 1;
  if (!util._isEmpty(data)) {
...
function writeCutomsData(deviceId: any, serviceId: any, characteristicId: any, data: any) {
  var obj: any = {}, frameControl = 0;
  sequenceControl = parseInt(sequenceControl as any) + 1;
  if (!util._isEmpty(data)) {
    obj = util.isSubcontractor(data, self.data.isChecksum, sequenceControl, self.data.isEncrypt);
    ...
    var ssidData = getCharCodeat(self.data.customData as any);
...
function writeGetNearRouterSsid(deviceId: any, serviceId: any, characteristicId: any, data: any) {
  sequenceControl = parseInt(sequenceControl as any) + 1;
...
function writeRouterSsid(deviceId: any, serviceId: any, characteristicId: any, data: any) {
  var obj: any = {}, frameControl = 0;
  sequenceControl = parseInt(sequenceControl as any) + 1;
  if (!util._isEmpty(data)) {
    obj = util.isSubcontractor(data, self.data.isChecksum, sequenceControl, self.data.isEncrypt);
...
function writeDevicePwd(deviceId: any, serviceId: any, characteristicId: any, data: any) {
  var obj: any = {}, frameControl = 0;
  sequenceControl = parseInt(sequenceControl as any) + 1;
  if (!util._isEmpty(data)) {
    obj = util.isSubcontractor(data, self.data.isChecksum, sequenceControl, self.data.isEncrypt);
  ...
  wx.writeBLECharacteristicValue({
    ...
    success: function (res) {
      if (obj.flag) {
        ...
        writeDeviceEnd(deviceId, serviceId, characteristicId);
      }
    },
    fail: function (res) { }
  })
...
function writeDeviceEnd(deviceId: any, serviceId: any, characteristicId: any) {
  sequenceControl = parseInt(sequenceControl as any) + 1;
  var frameControl = util.getFrameCTRLValue(self.data.isEncrypt, false, util.DIRECTION_OUTPUT, false, false);
  var value = util.writeData(util.PACKAGE_CONTROL_VALUE, util.SUBTYPE_END, frameControl, sequenceControl, 0, null);
  var typedArray = new Uint8Array(value)
  wx.writeBLECharacteristicValue({
    deviceId: deviceId,
    serviceId: serviceId,
...
function isTargetDevice(dev: any) {
  const name = dev.name || dev.localName || ''
  return name.startsWith('Xiaozhi-')
}
...
function init() {
  ...
  mDeviceEvent.listenStartDiscoverBle(true, function (options: any) {
                    ...
                    success: function (res) {
                      ...
                      wx.onBluetoothDeviceFound(function (devices: any) {
                        ...
                        if (devices.deviceId) {
                          ...
                          if (isnotexist && isTargetDevice(devices)) {
                        ...
                        // 2
                          ...
                          if (isnotexist && isTargetDevice(devices.devices[0])) {
                        ...
                        // 3
                          ...
                          if (isnotexist && isTargetDevice(devices[0])) {
  ...
  mDeviceEvent.listenConnectBle(true, function (options: any) {
  ...
  mDeviceEvent.listenInitBleEsp32(true, function (options: any) {
    sequenceControl = 0;
    sequenceNumber = -1;
    self = {
      data: {
    ...
    self.data.deviceId = options.deviceId
    console.log("nmnmnnmnm: ", deviceId)
  ...
  mDeviceEvent.listenSendRouterSsidAndPassword(true, function (options: any) {
  ...
  mDeviceEvent.listenSendCustomData(true, function (options: any) {
  ...
  mDeviceEvent.listenSendGetNearRouterSsid(true, function (options: any) {
    writeGetNearRouterSsid(self.data.deviceId, self.data.service_uuid, self.data.characteristic_write_uuid, null);
  })

}

function getList(arr: any, totalLength: any, curLength: any) {
